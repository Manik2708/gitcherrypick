...
| 2026-08-15 | **Amended by [ADR-0008](ADR-0008-discovery-http-surface.md)** — fixes the discovery HTTP surface ADR-0005 never specified. Shortlisting becomes **two-phase**: staging discloses nothing and `confirm` creates every contact request in one transaction, irreversibly. A notified entry can never be removed. Rank is global and the view filters it. |
